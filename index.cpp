...
void display () {
    ...
    glMatrixMode(GL_MODELVIEW);//stack depth 32//

	if (isWire) //blue print
		glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); 
		else 
			glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);//1st draw poly(outline) 2nd fill poly//

    glPushMatrix();//push matrix stack by 1 and duplicating
...
void drawMotorcycle()
{
    ...
    glPushMatrix();
        ...
        glScalef(1.4, 0.8, 1.0);//multiply the curr matrix with genral scale matrix
        glutSolidSphere(1,8,8);//radius,slices(no of subdivision around z axis),stack(subdivision along z axis)
        glPopMatrix();//store unscaled matrix (push n pop)
    ...
    glPushMatrix();
       glRotatef(90, 1, 0, 0);
        ...
        glCallList(pipe);//execute the display list specifies the integer name
...
void keyInput(unsigned char key, int x, int y)
{
   switch(key) 
   {
    ...
    case 'F': distance = (distance == 4)? 4:distance-1; break;
    case 'b': distance = (distance == 4)? 4:distance+1; break;
